...
def Greedy_Column_Generation(Base=None,Altura=None,x=-200,y=200,peças='fu',draw=False,escala=None,plot=False,passo=1,peso1_coluna = 2, peso2_coluna = 1, peso3_coluna = 2, peso1_pecas = 0, peso2_pecas = 1, verificacao_avancada = False):
    ...
    while not acabou:
        bases = []
        res = gerar_coluna(i,int(adicional),draw) #GERA COLUNAS ENQUANTO FOR POSSIVEL GERAR COLUNAS
        i+=1
        if res is not None:
            adicional+=res
        else:
...
def gerar_coluna(n,adicional,draw):
    ...
    graus = [0]
    colunas = []
    for j in range(0, len(lista_poligonos), 1): #PERCORRE TODAS AS PEÇAS EM TODAS AS ROTAÇÃO PARA GERAR A COLUNA
        turtle.tracer(0)
        dado1.clear()
        dado1.penup()
        dado1.goto(-925,100)
        ...
        for grau in graus:
            for i in range(adicional, base, 1):
                ...
                if tamanho > len(lista_poligonos):
                    ...
                    AF = exact_fit(0, int(adicional), int(adicional2), 0, draw, 0.83, base, altura, False) #EXECUTA A VARREDURA PARA ENCONTRAR A PRIMEIRA POSIÇÃO POSSIVEL PARA A PEÇA
    ...
    if colunas: #APÓS TESTAR TODAS AS COLUNAS, POSICIONA A MELHOR
...
##########################################################################################
##### AS DEMAIS FUNÇÕES SÃO AUXILIARES, PARA ESTRUTURAR AS CONTAS E A INTERFACE GRAFICA###
##########################################################################################
